...
class TokenRefreshErrorHandlingTest(TestCase):
    ...
    def setUp(self):
        ...
        self.social_post_url = reverse('demo:social_post', kwargs={
            'provider': 'twitter',
            'slug': self.token.slug
        })
    ...
    def test_social_post_token_refresh_error_shows_reauth_template(self, mock_get_provider):
        ...
        mock_provider_instance.get_valid_token.side_effect = TokenRefreshError(
            "Token refresh failed for twitter. Re-authorization may be required.",
            provider='twitter'
        )
        ...
        self.assertEqual(str(messages[0]), 
                        "Your Twitter connection has expired. Please reconnect to continue posting.")
    ...
    def test_social_post_unsupported_provider(self, mock_get_provider):
        ...
        url = reverse('demo:social_post', kwargs={
            'provider': 'unsupported',
            'slug': unsupported_token.slug
        })
    ...
    def test_social_post_token_not_found(self, mock_get_provider):
        ...
        fake_url = reverse('demo:social_post', kwargs={
            'provider': 'twitter',
