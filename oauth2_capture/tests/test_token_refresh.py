...
class TokenRefreshErrorHandlingTest(TestCase):
    ...
    def test_update_token_failure_raises_exception(self, mock_update, mock_user_info, mock_refresh):
        """Test that update_token failure raises TokenRefreshError"""
    ...
    def test_valid_token_unchanged(self):
        """Test that valid tokens are returned without refresh attempt"""
        # Create a non-expired token with different user_id to avoid unique constraint
